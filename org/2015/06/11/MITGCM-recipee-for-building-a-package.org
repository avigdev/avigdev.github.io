...
| MYPACK\_OPTIONS.h           | package specific MACRO option defs                                 |
| MYPACK\_PARAMS.h            | package parameters and their common block  (read from data.mypack) |
|-----------------------------+--------------------------------------------------------------------|
| code                        |                                                                    |
|-----------------------------+--------------------------------------------------------------------|
| mypack\_calc.F              | interface for mitgcnuv (this is what the model's core calls)       |
| mypack\_check.F             | check dependencies/conflicts with other packages                   |
| mypack\_diagnostics\_init.F | define diagnostics related to the package                          |
| mypack\_init\_varia.F       | initialize MYPACK parameters and variables                         |
| mypack\_output.F            | create diagnostic outputs                                          |
| mypack\_readparms.F         | parse data.mypack                                                  |
| mypack\_routines.F          | routines that implement double diffusion parametrization schemes   |
|-----------------------------+--------------------------------------------------------------------|
